...
export function configureCommonServices(container: Container): Container {
  dotenv.config()
  container.bind<any>(commonServiceTypes.EnvironmentVariables).toConstantValue(process.env);

  container.bind<any>(commonServiceTypes.DbConnection).toConstantValue(mongoose);
  ...
  container.bind<IConfigService>(commonServiceTypes.IConfigService).to(ConfigService);
  ...
  container.bind<IReflectionService>(commonServiceTypes.IReflectionService).to(ReflectionService);

  container.bind<IErrorHandlerService>(commonServiceTypes.IErrorHandlerService).to(ErrorHandlerService);

  container.bind<IRouteHandlerService>(commonServiceTypes.IRouteHandlerService).to(RouteHandlerService);
